"""基于 questionary 的复选框，支持「全选」行与组内条目在界面上的联动勾选。

实现要点：questionary 用 choice.value in ic.selected_options 渲染 [x]；
勾选全选时同时把组内各子项的 value 加入 selected_options，即可同步显示。
派生自 questionary.prompts.checkbox（MIT），仅改写空格切换逻辑。
"""
...
def _is_all_group_value(v: Any) -> bool:
    return isinstance(v, tuple) and len(v) == 2 and v[0] == ALL_SENTINEL


def _strip_all_groups_containing(ic: InquirerControl, member_value: Any) -> None:
    """取消某个子项勾选时，去掉包含它的「全选」行选中状态。"""
    for v in list(ic.selected_options):
        if _is_all_group_value(v) and member_value in v[1]:
...
def grouped_checkbox(
    ...
    validate: Callable[[List[Any]], Union[bool, str]] = lambda a: True,
    ...
    **kwargs: Any,
    ...
    def toggle(_event):
        ...
        if _is_all_group_value(pointed_choice):
            child_keys: Tuple[Any, ...] = pointed_choice[1]
            if pointed_choice in ic.selected_options:
                ic.selected_options.remove(pointed_choice)
                for k in child_keys:
                    while k in ic.selected_options:
                        ic.selected_options.remove(k)
            else:
                ic.selected_options.append(pointed_choice)
                for k in child_keys:
                    if k not in ic.selected_options:
                        ic.selected_options.append(k)
            perform_validation(get_selected_values())
            return

        if pointed_choice in ic.selected_options:
            ic.selected_options.remove(pointed_choice)
            _strip_all_groups_containing(ic, pointed_choice)
        else:
            ic.selected_options.append(pointed_choice)
